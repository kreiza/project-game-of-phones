...
class NoteRepository(AbstractRepository[Note]):
    ...
    def add(self, note: Note) -> Note:
        ...
        with SessionLocal() as session:
            ...
            orm_note = NoteORM(title=note.title, content=note.content, created_at = note.created_at, updated_at = note.updated_at, audit_message = note.audit_message, tags=tag_objs)
            session.add(orm_note)
            session.commit()
            session.refresh(orm_note)
            return Note(id=orm_note.id, title=orm_note.title, content=orm_note.content, created_at=orm_note.created_at, updated_at=orm_note.updated_at, audit_message = orm_note.audit_message, tags=[t.name for t in orm_note.tags])
    ...
    def list_all(self) -> List[Note]:
        ...
        with SessionLocal() as session:
            ...
            return [Note(id=n.id, text=n.text, title=n.title, content=n.content, created_at=n.created_at, updated_at=n.updated_at, audit_message = n.audit_message, tags=[t.name for t in n.tags]) for n in orm_notes]
